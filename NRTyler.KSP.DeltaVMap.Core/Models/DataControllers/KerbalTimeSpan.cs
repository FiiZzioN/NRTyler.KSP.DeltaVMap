...
namespace NRTyler.KSP.DeltaVMap.Core.Models.DataControllers
{
    public class KerbalTimeSpan
    {
        ...
        private void InitializeSuffixes()
        {
            ...
            AddSuffix("SECONDS", new Suffix<ScalarValue>(() => this.span));
        ...
        private ScalarValue CalculateYear()
        {
            ...
            {
                return (int)Math.Floor(this.span / SECONDS_IN_KERBIN_YEAR) + 1;
            }
            return (int)Math.Floor(this.span / SECONDS_IN_EARTH_YEAR) + 1;
        }
        ...
        {
            return (int)Math.Floor(this.span % SecongsPerYear / SecondsPerDay) + 1;
        }

        private ScalarValue CalculateHour()
        {
            return (int)Math.Floor(this.span % SecondsPerDay / SecondsPerHour);
        }

        private ScalarValue CalculateMinute()
        {
            return (int)Math.Floor(this.span % SecondsPerHour / SECONDS_IN_MINUTE);
        }

        private ScalarValue CalculateSecond()
        {
            return (int)Math.Floor(this.span % SECONDS_IN_MINUTE);
        }
        ...
        public override bool Equals(object obj)
        {
            ...
            if (compareType.IsInstanceOfType(obj))
            {
                ...
                return this.span == t.ToUnixStyleTime();
            }
            return false;
        }
        ...
        {
            return string.Format("TIME({0:0})", this.span);
        }
        ...
        {
            this.span = Convert.ToDouble(dump[DumpSpan]);
        }
